<!--header-->
<?php require_once 'includes/modules/header.php'; ?>
    <style>
        html, body {
            overflow: hidden;
        }
        #header {
            top: -120;
        }
    </style>
	<script type="text/javascript" src="scripts/jquery.pagepiling.min.js"></script>
	<script type="text/javascript">
		$(document).ready(function() {
			/*
			* Plugin intialization
			*/
	    	$('#pagepiling').pagepiling({
	          	direction: 'vertical',
	    		menu: '#menu',
	    		anchors: ['page1', 'page2'],
			    navigation: {
			    	'position': 'right',
                    'bulletsColor': '#fff',
			   		'tooltips': ['image', 'see more']
			   	},
			    afterRender: function(){
			    	$('#pp-nav').addClass('custom');
			    },
			    afterLoad: function(anchorLink, index){
			    	if(index>1){
			    		$('#pp-nav').removeClass('custom');
			    	}else{
			    		$('#pp-nav').addClass('custom');
			    	}
			    }
			});
	    });
    </script>
	<div id="pagepiling">
	    <div class="section dg ct" id="section1">
            <div class="box1 ct"></div>
            <div class="box1 ct"></div>
            <div class="box2 ct in">
                <p class="title bt wt">mini hamburger menu tooltip</p>
                <p class="wt">I've noticed that a lot of users still have no idea what the little three line menu buttons are for. They're generally associated with mobile applications, when they pop up on desktop sites it leaves people feeling lost. simply adding a bit of text you can ensure that far fewer people will have trouble navigating your applications.</p>
            </div>
            <div class="box3 in ct">
                <a href="images/labs/lab-2-1.gif" class="ot point">
                    <img src="images/labs/lab-2-1.gif" class="pi in" alt="iPhone 3d model" title="iPhone 3d model">
                </a>
                <?php include_once 'includes/modules/arrow-down-small.php'; ?>
            </div>
	    </div>
	    <div class="section dg ct" id="section2">
            <div class="box1 ct">
                <div class="box3 vb1 in">
                    <p class="title2 bt wt lt">download</p>
                    <p class="wt">all of the souce material used on this project is available to download below. feel free to use it for whatever you like.</p>
                </div>
                <div class="box1 vb1">
                    <div class="btn in o">
                        <a href="https://github.com/zacbeers/mini-hamburger-menu" target="_blank" class="point wt">
                            <p class="c">github</p>
                        </a>
                    </div>
                </div>
                <div class="box1 vb1 ct">
                    <<div class="btn in o">
                        <a href="labs.php" class="point wt">
                            <p class="c">< back to labs</p>
                        </a>
                    </div>
                </div>
            </div>
	    </div>
	</div>

