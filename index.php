...
$labs = DB::getInstance()->action("SELECT *", "labs", array('id', '>=', '0'), "ORDER BY id DESC");
            ...
            if(!$labs->count()) {
                ...
                foreach($labs->results() as $lab) {
                    if($l < 3) {
                    ?>
                    <a href="lab.php?id=<?php echo $lab->id; ?>"><div class="square" style="background: #<?php echo $lab->color; ?> url('<?php echo $lab->cover; ?>') no-repeat center center / cover;">
...
if(!$posts->count()) {
                ...
                foreach($posts->results() as $post) {
                    if($b < 1) {
                    ?>
                    <a href="post.php?id=<?php echo $post->id; ?>"><div class="rect" style="background: #<?php echo $post->color; ?> url('<?php echo $post->cover; ?>') no-repeat center center / cover;">
